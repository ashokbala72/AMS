...
if tab_index == 0:
    st_autorefresh(interval=10000, key='auto_summary_tab_refresh')
    st.session_state.ticket_context_df = get_ticket_data()
...
if tab_index == 0:
    st_autorefresh(interval=10000, key="summary_refresh")
    refresh_ticket_data()
...
with tabs[0]:
    ...
    triage_data = []
    for ticket in st.session_state.tickets[-15:]:
        ticket_id = ticket['id']
        priority = ["Low", "Medium", "High"][(int(ticket_id.split('-')[1]) % 3)]
        row = {"Ticket": ticket_id, "Priority": priority}
        for agent in agents:
    ...
    st.markdown(df_triage.to_html(escape=False, index=False, classes="ticket-table"), unsafe_allow_html=True)
    st.info("Status is auto-refreshed every 10 seconds. 🕐")
...
{context}"""
    ...
    for t in st.session_state.tickets:
        audit.append({
            ...
            "Color": t["color"],
            "Seconds in Stage": round(now - t["start_time"], 1)
        })
